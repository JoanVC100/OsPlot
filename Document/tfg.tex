...
\begin{itemize}
	\item Fent servir \cite[socat]{socat}, una eina que permet,
          ...
          un PTY. Això vol dir que un programa de \est{trigger}
...
l'element \est{canvas} de HTML5 que permet oferir una experiència
...
\item MCU\_N\_MOSTRES: Paquet que conté el nombre de mostres d'una
  finestra (2 bytes).
\item MCU\_FS: Retorna la freqüència de mostreig de l'\acro{ADC}.
...
reduir el soroll capturat per l'\acro{ADC}. La freqüència efectiva es
...
sèrie, mor el procés de GNUPlot\dots), o si la tasca de la \acro{CLI}
...
l'usuari (que arriben per \est{stdin}) i transmetre-les a la tasca
principal.  Per limitacions de tokio, en comptes de crear una tasca
per llegir la \est{stdin}, es crea un fil que llegeix l'entrada de
...
d'utilitzar protocols serials tradicionals com \acro{UART} o
\acro{SPI}, així com altres tecnologies de xarxa com \acro{TCP/IP} o
WebSockets, l'ús de \acro{SDR} pot proporcionar una solució més
